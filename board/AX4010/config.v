...
`define TERMINAL_SPEED 3'd3

// скорость второго последовательного интерфейса
`define UART2SPEED 3'd3
...
//****************************************
 ...
 //`define LTC                // признак наличия подсистемы управления таймером Line Time Clock (LTC)
 `define UMAP               // признак наличия подсистемы Unibus mapping.
                            // В режиме 256К ОЗУ подсистема отключается автоматически
// Определение объема памяти платы.
// Если закомментировать обе переменных, то включается полный объем 4М.
 `define RAM256              // ограничение объема памяти до 256К (18-битный адрес) МС1201.03
...
// `define RAM256              // ограничение объема памяти до 256К (18-битный адрес)
 `define RAM512              // ограничение объема памяти до 512К (19-битный адрес)
 //`define LTC                // признак наличия подсистемы управления таймером Line Time Clock (LTC)
